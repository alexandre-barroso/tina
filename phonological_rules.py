...
def generate_vowel_harmony_variants(variants):
    """
    Generate phonetic variants based on vowel harmony and reduction.
    
    For every occurrence of a vowel harmony marker ('I' or 'U') in the transcription, generate alternative variants:
      - 'I' is replaced by 'e' and 'i'
      - 'U' is replaced by 'o' and 'u'
    
    Additionally, if a preceding consonant is present (only 't' or 'd' in this example), include an extra
    set of alternatives:
      - For 't': retain 't' or transform to 'tʃ'
      - For 'd': retain 'd' or transform to 'dʒ'
    
    Args:
        variants (set): A set of transcription strings (e.g. generated by earlier steps in the pipeline).
    
    Returns:
        set: A new set of variants with vowel harmony modifications applied.
    """
    new_variants = set()
