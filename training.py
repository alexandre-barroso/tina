...
#############################
# Instantiate Latent Space
#############################
# Make sure the latent space dimension matches your encoder hidden dimension (ENC_HID_DIM)
latent_space_module = PhoneticLatentSpace(dim=ENC_HID_DIM, num_patterns=32)

#############################
# Vocab / Tokenization (unchanged)
#############################
...
#############################
# Dataset with Padding (unchanged)
#############################
class PaddedPhoneticDataset(Dataset):
    def __init__(self, data_path, max_char_len=30, max_syl_len=10, max_ph_len=12):
        self.max_char_len = max_char_len
        self.max_syl_len = max_syl_len  # total length for syllable teacher sequence (including bow & eow)
        ...
        for ortho, phon_str, exp_count in self.samples:
            ...
            # Build teacher syllable sequence including bow and eow.
            syl_seq = [self.bow_idx]
            # Build teacher stress sequence for the real syllables (excluding bow).
            stress_seq = []
            ph_lists = []
            ...
            # Append trailing dummy for stress (for the eow position); 
            # now stress_seq length = (# real syllables) + 1.
            stress_seq.append(0)

            # Pad syllable sequence to fixed length (max_syl_len).
            syl_seq = self._pad_seq(syl_seq, self.max_syl_len, self.pad_idx_syl)
            # Pad stress sequence to match the decoder input length (max_syl_len - 1,
            # since teacher forcing uses syll_seq[:-1] as input).
            stress_seq = self._pad_seq(stress_seq, self.max_syl_len - 1, 0)

            # Pad phoneme sequences to length max_syl_len - 1 (one per real syllable).
...
#############################
# Define Custom Focal Loss for Stress
#############################
...
# Losses with label smoothing (if supported, PyTorch 1.10+)
...
# New boundary token loss criterion
boundary_criterion = nn.CrossEntropyLoss(ignore_index=dataset.pad_idx_syl)

# Loss weights (you can later use adaptive weights as before or fixed weights)
loss_weights = {
    'syl': 0.30,
    'stress': 0.30,
    'phon': 0.20,
...
boundary_token_weight = 0.1  # Small weight for direct token-level boundary supervision
...
# Training Loop
#############################
# Use an exponential decay for teacher forcing.
decay_constant = 30.0
...
while True:
    ...
    # (Optional) Reset the latent space statistics for the epoch if desired:
    ...
    for batch_data in pbar:
        ...
        # Get the actual number of decoder steps from the output.
        dec_syl_steps = syllable_logits.size(1)
        
        # Slice the teacher syllable sequence so that it matches the decoder outputs.
        # Instead of taking the full syl_seq[:, 1:], we take only the first dec_syl_steps tokens.
        teacher_syl = syl_seq[:, 1:1 + dec_syl_steps]
        
        syl_seq_clamped = syl_seq[:, 1:dec_syl_steps + 1]
        # The teacher stress sequence is already produced without the bow token so it is of length dec_syl_steps.
        stress_seq_clamped = stress_seq[:, :dec_syl_steps]
        
        # Derive teacher boundary sequence:
        teacher_boundary_seq = (syl_seq_clamped == dataset.eow_idx).long()

        # 1) Syllable Loss (with label smoothing)
        # Flatten logits and teacher tokens for the syllable loss.
        ...
        # 2) Stress Loss (Focal Loss)
        # Compute the valid mask on the sliced teacher syllable sequence (which no longer includes the bow token)
        ...
        # --- Update latent space statistics for this batch ---
        # Use the encoder to get aggregated hidden state.
        with torch.no_grad():
            # Re-run the encoder on src to obtain encoder hidden states.
            _, enc_hidden, _ = model.encoder(src)
            # Use the last-layer hidden state as the aggregated representation.
            aggregated_hidden = enc_hidden[-1]  # shape: [B, ENC_HID_DIM]
            # Flatten teacher phoneme sequences: shape [B, max_syl_len * max_ph_len]
    ...
    # Save checkpoint and the updated latent space model at this epoch.
